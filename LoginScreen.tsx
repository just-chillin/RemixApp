...
export default class LoginScreen extends Component<{}, {email: string, password: string}> {
  ...
  componentWillMount = () => AsyncStorage.getItem('auth').then(res => {
    if (res !== null && this.shouldAutoLogin) {
      console.log(`BEARER TOKEN ${res} FOUND! The component should auto login here.`);
      this.goToApp();
    } else {
  ...
  login = () =>
    Promise.resolve(RESTService.validateCredentials(this.state.email, this.state.password).then(res => {
      if (res.ok) {
        console.log('Login Successful!!!!');
        AsyncStorage.setItem("auth", btoa(`${this.state.email}:${this.state.password}`));
        this.goToApp();
      } else {
